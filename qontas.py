...
def parse_cigar_for_variants(read, ref_seq_str, ignore_edges=3):
    # pysam cigartuples: 7=EQ (=), 8=X (mismatch)
...
def main():
    parser = argparse.ArgumentParser(description="QONTAS: Multi-Record Hybrid Pipeline")
    ...
    for reg in regions:
        ...
        run_cmd(f"vsearch --derep_fulllength {reg_fa} --output {reg_derep} --uc {reg_uc} --minuniquesize {args.mincount} --sizeout", "VSEARCH Denoising")

        # C. Map Unique Sequences (Sort & Index)
        ref_slice = os.path.join(args.outdir, f"{prefix}_ref_slice.fa")
        if not create_ref_slice_fasta(args.ref, reg['chrom'], reg['start'], reg['end'], ref_slice): continue
        
        unique_bam = os.path.join(args.outdir, f"{prefix}_unique.bam")
        run_cmd(f"minimap2 -a --eqx -x map-ont -t {args.threads} {ref_slice} {reg_derep} | samtools sort -o {unique_bam}", "Final Mapping")
    ...
    print(f"\nSUCCESS. Check results in {args.outdir}")
